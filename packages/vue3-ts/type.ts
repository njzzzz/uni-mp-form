...
export interface ChangeFn<T extends string | number | symbol> {
  changeConfig?: FormItem<T>['changeConfig']
  changeValue?: FormItem<T>['changeValue']
}
export type DependOnObj<T extends string | number | symbol> = Partial<Record<T, ChangeFn<T>>>
export type ReadonlyDependValue<T extends string | number | symbol> = DeepReadonly<{
  [K in T]: any;
}>
// 组件需要自己实现这些功能
export type WidgetPropsInFormItem = 'disabled' | 'readonly' | 'clearable'
export interface FormItem<T extends string | number | symbol = string> {
  /**
   * 表单项类型
   */
  widget?: Widget
...
> {
  ...
  border?: boolean
  config: Record<keyof T, FormItem<keyof T>>
  /**
   * 在进行显示隐藏操作时是否要保留值
   */
  cache?: boolean
